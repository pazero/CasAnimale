        ...
        <div className="flex flex-wrap justify-center h-full" style={{flex:"0 1 auto" }}>
          {company.map((product) => (
            <a
              ...
              style={{ border: "solid 1px #191A3E", color: "#191A3E", minWidth: "20rem"}}
              onClick={() => {navigate("/specialistpage/"+product._id);}}
            >
              <div>
                <div className="mb-2 rounded font-semibold text-2xl text-center" style={{color: "#191A3E"}}>
                  {product.name}
                </div>
                
                <div className="mb-2 mx-2">
                  <p className="font-semibold">Description: </p>
                  <div className="ml-4" style={{overflowWrap:"break-word", inlineSize:"20rem"}}>{product.description}</div>
                </div>
                
                <div className="mb-2 mx-2">
                  <p className=" font-semibold">Doctor: </p>
                  <p className="ml-4" style={{overflowWrap:"break-word", inlineSize:"20rem"}}>{product.owner}</p>
                </div>
                
                <div className="mb-2 mx-2">
                <p className="font-semibold">Cities: </p>
                <ul className="ml-4" style={{overflowWrap:"break-word", inlineSize:"20rem"}}> {product.cities.map((item) => (<li>{item}</li>))}</ul>
                </div>
              </div>
