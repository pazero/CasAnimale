<template class="antialiased text-gray-700 bg-gray-100">
  <div>
    <div v-if="!fetchDone">
      <button v-if="!fetchDone" type="button" id="get-quiz" @click="getQuestion" class="getQuizButton">
        Get some questions!
      </button>
    </div>
    <div v-else class="flex w-full h-screen justify-center items-center">
      <div class="w-full max-w-xl p-3">
        ...
        <div ref="gameState" class="bg-white p-12 rounded-lg shadow-lg w-full mt-8">
          <div v-if="idx < count">
...
export default {
  ...
  methods: {
    async getQuestion() {
      ...
      await fetch('https://opentdb.com/api.php?amount=' + this.count + '&category=27&type=multiple')
        .then((response) => {
          if (response.ok) {
